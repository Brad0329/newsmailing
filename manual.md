...
#### 이름 붙여 여러 리스트 저장
수신자 textarea 바로 위 툴바:
- **저장된 리스트 드롭다운**: 보관된 메일 리스트 목록 (각 항목에 인원수 표시)
- **[불러오기]**: 선택한 리스트를 수신자 textarea에 덮어쓰기 (기존 내용이 있으면 확인)
- **[삭제]**: 선택한 리스트 제거 (되돌릴 수 없음)
- **이름 입력 + [현재 내용 저장]**: 현재 수신자 textarea 내용을 해당 이름으로 저장 (이름 중복 시 덮어쓰기 확인)

저장 위치: `data\settings.json` 의 `mailing_lists` 배열.

활용 예:
- "VAN 회원 전체" / "주요 카드사" / "언론사 담당자" 등으로 그룹 분리
- 매일 보낼 때 드롭다운에서 그룹 선택 → [불러오기] → 필요시 추가·삭제 후 발송
