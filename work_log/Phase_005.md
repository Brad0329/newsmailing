...
- ~~언론사 도메인 매핑 추가 확장 (현재 30개 → 목표 50개 이상)~~ → 2026-04-22 완료 (아래 후속 작업 참조)
- 이메일 주소 정규식 유효성 검사 (현재는 형식 검증 없음)
- E2E 테스트 자동화 (현재 단위 테스트 + 수동 E2E)
- `SMTP_USE_TLS` 변수명 개선 리팩터
- 도메인 매핑에서 누락된 서브도메인 케이스 추가 (예: `biz.chosun.com`은 strip 규칙에 의해 `chosun.com`으로 축약되어 조선비즈가 조선일보로 매핑되는 문제)

---

## 후속 작업 — 도메인 매핑 외부화 + 확장 (2026-04-22)

### 결정의 이유

1. **파일 분리 이유**: 기존 `naver_client.py` 모듈 상수로 30개가 하드코딩되어 매핑 추가할 때마다 코드 수정 + 재빌드 필요. 매핑 데이터는 시크릿이 아니고 운영 중 확장 가능성이 높은 설정성 데이터이므로 외부 JSON으로 분리.
2. **`settings.json`과 분리한 이유**: `settings.json`은 사용자별 가변 상태(수신자/제목/서두)라 `.gitignore` 대상. `domain_map.json`은 앱이 배포 시점에 동반하는 baseline이라 git 추적 대상으로 두어 배포 간 버전 관리 가능. 성격이 달라 파일 분리.
3. **세 번째 옵션(DB) 배제 이유**: 단일 사용자 소규모 운영, 매핑 규모 100여개 수준이라 JSON 파일로 충분. DB 도입은 과한 복잡도.
4. **캐시 전략**: 모듈 로드 시 1회만 파일을 읽어 `_domain_map_cache`에 보관. 파일 수정 시 재로드하려면 `reload_domain_map()` 명시 호출. `extract_source`가 기사당 한 번씩 호출되는 경로라 매 호출 디스크 I/O는 피함.

### 확장 규모

- 초기 30개 → **96개**로 확장.
- 카테고리: 통신사(4), 종합일간지(11), 경제지(21), IT/전자(11), 방송(9), 시사/대안(10), 스포츠(4), 연예(4), 영자(2), 지역(13), 업계전문(3), 온라인(2). 매핑 근거가 확실한 도메인만 포함(불확실한 도메인은 제외, 도메인 문자열 폴백으로 처리).

### 외부 제약 조건 (운영 메모)

- **`data/domain_map.json`은 git 추적 대상**. 반면 `data/settings.json`, `data/history.json`은 `.gitignore`. 배포 시 exe 옆에 `data/domain_map.json`을 함께 배치해야 하며, 없으면 빈 dict 폴백 → 모든 출처가 도메인 문자열로 노출된다.
- **매핑 추가 시**: JSON 파일만 편집하면 됨. 코드 수정/재빌드 불필요. 단, 실행 중이던 서버는 재시작(또는 `reload_domain_map()` 호출)해야 반영.
- **key 표기 규칙**: 소문자, 서브도메인 제거 후 형태. `extract_source`가 `^(www|news|biz|sports|m|imnews|v)\.` 를 strip하므로 key도 strip 결과에 맞춰야 함.
- **서브도메인 충돌**: `biz.chosun.com`(조선비즈) 같은 케이스는 strip 규칙에 의해 `chosun.com`(조선일보)로 축약되어 잘못 매핑됨. 매핑 테이블만 확장해도 해결 안 됨 — strip 규칙 자체를 케이스별로 손봐야 함(별도 이월 항목으로 기존에 명시됨).

### 향후 운영상 주의점

- 매뉴얼에 "언론사 이름 추가하려면 `data/domain_map.json` 편집" 안내 추가 권장.
- 관리 UI(웹 화면에서 매핑 추가/편집) 필요 시 별도 Phase로 진행. 현재는 파일 직접 편집으로 대응.
